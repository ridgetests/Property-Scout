...
| Aug 2026 | TASK E **DONE**: `make_building_heights.py` + **Build building heights** workflow. Per eligible candidate, WCS GetCoverage DTM + first-return DSM for a small box, nDSM = DSM − DTM, records ridge/mean height → `docs/building_heights.json`. Barns map shows the height and a **"Hide house-like (LIDAR)"** toggle (drops ridge ≥9.5 m — probably a house, not a barn). ~2 spaced WCS requests per eligible candidate, behind a breaker; never commits raster. |
