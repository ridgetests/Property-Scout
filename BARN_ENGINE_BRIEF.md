...
| Aug 2026 | **Disused/derelict** expansion. `make_footprints.py` now captures OSM lifecycle tags (`disused:building`, `abandoned`, `ruins`, `historic=ruins`, `building=ruins/collapsed`, `building:condition=derelict`) into a `d` flag. `make_barn_candidates.py` treats derelict buildings as prime candidates: size floor relaxed to 100 m² for agri/derelict, kept even off-field, +0.15 score uplift. Barns map shows a "⚠ disused/derelict" badge. (Finds MAPPED derelict buildings; unmapped/tree-hidden ones need LIDAR anomaly detection — a future build.) |
| Aug 2026 | **Reframe: barn engine → convertible-building engine** (user: "any building that can be converted into the home I want"). `make_footprints.py` captures `amenity` and classifies each building's PURPOSE (`cls`): religious / education / civic / commercial / industrial / agricultural. `make_barn_candidates.py` surfaces these non-residential types (not just barns), each with a **conversion route** (Class Q, Class MA, full planning) and a convertibility prior (barn 1.0, chapel 0.85, school 0.80, hall 0.75, commercial 0.60, industrial 0.45). Class-Q designation gate now only *bars* agricultural/field/derelict; other routes show designations as constraints, not a bar. Map: title = building class, a Route line, blue pins for non-Class-Q routes. Deliberately excludes plain town retail/office unless disused, to avoid noise. Needs a re-run of **Build barn footprints** → **Build barn candidates**. |
