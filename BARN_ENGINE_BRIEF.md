...
| Aug 2026 | TASK E started: **LIDAR probe**. Research: the old ESRI portal (EAlidaR) is retired; new **keyless DEFRA catalogue API** (`.../backend/catalog/api/tiles/collections/survey/search`, POST a lon/lat polygon → tile `uri`s, download `{uri}?subscription-key=public`), OGL, no account. WCS bbox route also exists. GEE asset `UK/EA/ENGLAND_1M_TERRAIN/2022` **confirmed real** (fallback). Only unknown = whether Actions IPs are served. `make_lidar_probe.py` + read-only **Probe LIDAR** workflow does one search + one tile download with loud diagnostics + a verdict. Awaiting probe run before building the nDSM→per-building-height pipeline. |
